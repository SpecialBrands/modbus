...
func (mb *asciiPackager) Encode(pdu *ProtocolDataUnit) (adu []byte, err error) {
	...
	if err = writeHex(&buf, []byte{mb.ClientId, pdu.FunctionCode}); err != nil {
		return
	}
	...
	var lrc lrc
	lrc.reset()
	lrc.pushByte(mb.ClientId).pushByte(pdu.FunctionCode).pushBytes(pdu.Data)
...
// Verify verifies response length, frame boundary and client id.
func (mb *asciiPackager) Verify(aduRequest []byte, aduResponse []byte) (err error) {
	...
	if responseVal != requestVal {
		err = fmt.Errorf("modbus: response client id '%v' does not match request '%v'", responseVal, requestVal)
		return
	}
	return
}
